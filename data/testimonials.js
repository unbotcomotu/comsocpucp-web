export const testimonials = [
  {
    name: "Coriss Ambady",
    title: "Financial Analyst",
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio.",
  },
  {
    name: "Cory Zamora",
    title: "Marketing Specialist",
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio.",
  },
  {
    name: "Nikolas Brooten",
    title: "Sales Manager",
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio.",
  },
];

export const testimonials2 = [
  {
    name: "Coriss Ambady",
    title: "Financial Analyst",
    quote:
      "Vivamus sagittis lacus augue laoreet rutrum faucibus auctor vestibulum ligula porta felis, euismod semper cras justo odio.",
    imageSrc: "/assets/img/avatars/te1.jpg",
    imageSrcSet: "/assets/img/avatars/te1@2x.jpg 2x",
  },
  {
    name: "Cory Zamora",
    title: "Marketing Specialist",
    quote:
      "Vivamus sagittis lacus augue laoreet rutrum faucibus auctor vestibulum ligula porta felis, euismod semper cras justo odio.",
    imageSrc: "/assets/img/avatars/te2.jpg",
    imageSrcSet: "/assets/img/avatars/te2@2x.jpg 2x",
  },
  {
    name: "Nikolas Brooten",
    title: "Sales Manager",
    quote:
      "Vivamus sagittis lacus augue laoreet rutrum faucibus auctor vestibulum ligula porta felis, euismod semper cras justo odio.",
    imageSrc: "/assets/img/avatars/te3.jpg",
    imageSrcSet: "/assets/img/avatars/te3@2x.jpg 2x",
  },
];

export const slidesData = [
  {
    id: 1,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio consectetur adipiscing dapibus.",
    imageSrc: "/assets/img/avatars/te1.jpg",
    imageSrcSet: "/assets/img/avatars/te1@2x.jpg 2x",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 2,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio consectetur adipiscing dapibus.",
    imageSrc: "/assets/img/avatars/te2.jpg",
    imageSrcSet: "/assets/img/avatars/te2@2x.jpg 2x",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    id: 3,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum ligula porta felis euismod semper. Cras justo odio consectetur adipiscing dapibus.",
    imageSrc: "/assets/img/avatars/te3.jpg",
    imageSrcSet: "/assets/img/avatars/te3@2x.jpg 2x",
    name: "Nikolas Brooten",
    role: "Sales Manager",
  },
];

export const testimonials3 = [
  {
    id: 1,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te7.jpg",
    avatarSrcSet: "/assets/img/avatars/te7@2x.jpg 2x",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 2,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te8.jpg",
    avatarSrcSet: "/assets/img/avatars/te8@2x.jpg 2x",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    id: 3,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te9.jpg",
    avatarSrcSet: "/assets/img/avatars/te9@2x.jpg 2x",
    name: "Nikolas Brooten",
    role: "Sales Manager",
  },
  {
    id: 4,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te10.jpg",
    avatarSrcSet: "/assets/img/avatars/te10@2x.jpg 2x",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 5,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te11.jpg",
    avatarSrcSet: "/assets/img/avatars/te11@2x.jpg 2x",
    name: "Jackie Sanders",
    role: "Investment Planner",
  },
  {
    id: 6,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque.",
    avatarSrc: "/assets/img/avatars/te12.jpg",
    avatarSrcSet: "/assets/img/avatars/te12@2x.jpg 2x",
    name: "Laura Widerski",
    role: "Sales Specialist",
  },
];

export const testimonials4 = [
  {
    id: 1,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis natoque penatibus.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
    imageSrc: "/assets/img/avatars/te1.jpg",
    imageSrcSet: "/assets/img/avatars/te1@2x.jpg 2x",
  },
  {
    id: 2,
    quote:
      "Fusce dapibus, tellus ac cursus tortor mauris condimentum fermentum massa justo sit amet. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Cras mattis consectetur purus sit amet fermentum. Aenean lacinia bibendum nulla sed.",
    name: "Cory Zamora",
    role: "Marketing Specialist",
    imageSrc: "/assets/img/avatars/te2.jpg",
    imageSrcSet: "/assets/img/avatars/te2@2x.jpg 2x",
  },
  {
    id: 3,
    quote:
      "Curabitur blandit tempus porttitor. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Nullam quis risus eget porta ac consectetur vestibulum. Donec sed odio dui consectetur adipiscing elit.",
    name: "Nikolas Brooten",
    role: "Sales Manager",
    imageSrc: "/assets/img/avatars/te3.jpg",
    imageSrcSet: "/assets/img/avatars/te3@2x.jpg 2x",
  },
  {
    id: 4,
    quote:
      "Etiam adipiscing tincidunt elit convallis felis suscipit ut. Phasellus rhoncus tincidunt auctor. Nullam eu sagittis mauris. Donec non dolor ac elit aliquam tincidunt at at sapien. Aenean tortor libero condimentum ac laoreet vitae.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
    imageSrc: "/assets/img/avatars/te4.jpg",
    imageSrcSet: "/assets/img/avatars/te4@2x.jpg 2x",
  },
  {
    id: 5,
    quote:
      "Maecenas sed diam eget risus varius blandit sit amet non magna. Cum sociis natoque penatibus magnis dis montes, nascetur ridiculus mus. Donec sed odio dui. Nulla vitae elit libero.",
    name: "Jackie Sanders",
    role: "Investment Planner",
    imageSrc: "/assets/img/avatars/te5.jpg",
    imageSrcSet: "/assets/img/avatars/te5@2x.jpg 2x",
  },
  {
    id: 6,
    quote:
      "Donec id elit non mi porta gravida at eget metus. Nulla vitae elit libero, a pharetra augue. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.",
    name: "Laura Widerski",
    role: "Sales Specialist",
    imageSrc: "/assets/img/avatars/te6.jpg",
    imageSrcSet: "/assets/img/avatars/te6@2x.jpg 2x",
  },
];

export const testimonials5 = [
  {
    id: 1,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio dapibus facilisis sociis.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 2,
    quote:
      "Fusce dapibus, tellus ac cursus tortor mauris condimentum fermentum massa justo sit amet. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Cras mattis consectetur purus sit amet fermentum.",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    id: 3,
    quote:
      "Curabitur blandit tempus porttitor. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Nullam quis risus eget porta ac consectetur vestibulum. Donec sed odio dui consectetur.",
    name: "Nikolas Brooten",
    role: "Sales Manager",
  },
  {
    id: 4,
    quote:
      "Etiam adipiscing tincidunt elit convallis felis suscipit ut. Phasellus rhoncus tincidunt auctor. Nullam eu sagittis mauris. Donec non dolor ac elit aliquam tincidunt at at sapien.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 5,
    quote:
      "Maecenas sed diam eget risus varius blandit sit amet non magna. Cum sociis natoque penatibus magnis dis montes, nascetur ridiculus mus. Donec sed odio dui.",
    name: "Jackie Sanders",
    role: "Investment Planner",
  },
  {
    id: 6,
    quote:
      "Donec id elit non mi porta gravida at eget metus. Nulla vitae elit libero, a pharetra augue. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur.",
    name: "Laura Widerski",
    role: "Sales Specialist",
  },
];

export const testimonials6 = [
  {
    id: 1,
    quote:
      "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper. Cras justo odio, dapibus ac facilisis sed posuere.",
    imageSrc: "/assets/img/avatars/te1.jpg",
    imageSrcSet: "/assets/img/avatars/te1@2x.jpg 2x",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 2,
    quote:
      "Donec id elit non mi porta gravida at eget metus. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Duis mollis, est non commodo luctus.",
    imageSrc: "/assets/img/avatars/te2.jpg",
    imageSrcSet: "/assets/img/avatars/te2@2x.jpg 2x",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    id: 3,
    quote:
      "Nisi erat porttitor ligula, eget lacinia odio sem nec elit. Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Donec id elit non mi porta gravida.",
    imageSrc: "/assets/img/avatars/te3.jpg",
    imageSrcSet: "/assets/img/avatars/te3@2x.jpg 2x",
    name: "Nikolas Brooten",
    role: "Sales Manager",
  },
  {
    id: 4,
    quote:
      "Etiam porta sem malesuada magna mollis euismod. Etiam porta sem malesuada magna mollis euismod. Donec id elit non mi porta gravida at eget metus posuere consectetur.",
    imageSrc: "/assets/img/avatars/te4.jpg",
    imageSrcSet: "/assets/img/avatars/te4@2x.jpg 2x",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 5,
    quote:
      "Maecenas faucibus mollis interdum. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Praesent commodo cursus magna, vel scelerisque.",
    imageSrc: "/assets/img/avatars/te5.jpg",
    imageSrcSet: "/assets/img/avatars/te5@2x.jpg 2x",
    name: "Jackie Sanders",
    role: "Investment Planner",
  },
  {
    id: 6,
    quote:
      "Donec id elit non mi porta gravida at eget metus. Cras justo odio, dapibus ac facilisis in, egestas eget quam. Maecenas sed diam eget risus varius blandit sit amet non magna.",
    imageSrc: "/assets/img/avatars/te6.jpg",
    imageSrcSet: "/assets/img/avatars/te6@2x.jpg 2x",
    name: "Laura Widerski",
    role: "Sales Specialist",
  },
];

export const testimonials7 = [
  {
    text: "Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta. Cras mattis consectetur.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    text: "Fusce dapibus, tellus ac cursus tortor mauris condimentum fermentum massa justo sit amet purus sit amet fermentum.",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    text: "Curabitur blandit tempus porttitor. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor eu rutrum. Nulla vitae libero.",
    name: "Nikolas Brooten",
    role: "Sales Manager",
  },
  {
    text: "Etiam adipiscing tincidunt elit convallis felis suscipit ut. Phasellus rhoncus eu tincidunt auctor nullam rutrum, pharetra augue.",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
];

export const testimonials8 = [
  {
    id: 1,
    widthClasses: "md:w-6/12 lg:w-6/12 xl:w-5/12 w-full",
    bgColor: "!bg-[#fef3e4]",
    quote: "“Cum sociis natoque penatibus et magnis dis parturient montes.”",
    name: "Coriss Ambady",
    role: "Financial Analyst",
  },
  {
    id: 2,
    widthClasses: "xl:w-6/12 lg:w-6/12 md:w-6/12 w-full",
    bgColor: "!bg-[#fae6e7]",
    quote:
      "“Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vestibulum id ligula porta felis euismod semper.”",
    name: "Cory Zamora",
    role: "Marketing Specialist",
  },
  {
    id: 3,
    widthClasses: "md:w-6/12 lg:w-6/12 xl:w-5/12",
    bgColor: "!bg-[#eaf3ef]",
    quote:
      "“Donec id elit non porta gravida at eget metus. Duis mollis est commodo luctus, nisi erat porttitor.”",
    name: "Barclay Widerski",
    role: "Sales Specialist",
  },
  {
    id: 4,
    widthClasses: "xl:w-6/12 lg:w-6/12 md:w-6/12 w-full",
    bgColor: "!bg-[#e0e9fa]",
    quote:
      "“Nisi erat porttitor ligula, eget lacinia odio sem nec elit. Aenean eu leo pellentesque ornare.”",
    name: "Jackie Sanders",
    role: "Investment Planner",
  },
];
